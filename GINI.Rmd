...
author: |
 | Lulu Jiang (<lulu.jiang@dpag.ox.ac.uk>), Hai Fang (<23versify@gmail.com>)
date: "`r Sys.Date()`"
url: 'https\://luca0330.github.io/gini/'
github-repo: "luca0330/gini"
...
> **Step 1: Load the packages and import human genetic interaction data (see `Materials` above).**
...
> **Step 7. Summarise the number of genes found in each module.**
```{r, echo=TRUE}
...
> **Step 9. Explore enrichment results for a module.**
```{r, echo=TRUE}
...
> **Step 12. Visualise and compare enrichment results between modules.**
```{r, echo=TRUE}
...
We illustrate how to perform integrative analysis combining genetic interactions with gene expression data; such strategy is computationally promising given the current technological limits in experimentally generating tissue-specific interactions on a genome scale, particularly for humans. One way doing so is to trim the generic genetic interactions to node genes expressed in a specific tissue. Such trimming allows identification of a genetic interaction network in the whole blood (`Step 2`), from which a subnetwork with a desired number of interconnected genes is further identified that tend to be highly expressed (`Step 3`). We also demonstrate how to highlight the subnetwork within the parent network (`Step 4`). For the interpretation of the subnetwork identified, we illustrate how to perform phenotype enrichment analysis using mammalian phenotype ontology, a tree-like structure containing well-defined terms that are used to annotate mouse knock-out phenotypes (`Step 6`). 
...
# converted into two tibbles "vertices" and "edges"
ig.BioGRID_genetic <- xRDataLoader("ig.BioGRID_genetic", guid=guid)
ig.BioGRID_genetic %>% igraph::as_data_frame("vertices") %>% as_tibble() -> vertices
...
> **Step 2. Identify a genetic interaction network in the whole blood.**
...
# return two data frames "nodes" and "links"
...
vertices %>% inner_join(df_blood, by=c("name"="Symbol")) %>% 
...
> **Step 3: Further identify a subnetwork with highly expressed genes.**
...
# but appended with a node attribute ("mark") and edge attribute ("mark") 
...
# visualise the network highlighted by the subnetwork
# nodes placed by coordinates
# nodes sized by degree
...
> **Step 5. Display expression levels for genes in the subnetwork.**
```{r, echo=TRUE}
...
> **Step 6. Perform phenotype enrichment analysis for genes in the subnetwork.**
```{r, echo=TRUE}
...
In this case, we introduce a more advanced analysis workflow, showing how to identify tissue clusters (`Steps 2 and 7`) and differential interactions (`Steps 8-10`) based on the integration of genetic interactions with gene expression. 

> **Step 1: Load the packages and import human genetic interaction data as well as human tissue gene expression data (see `Materials` above).**
...
> **Step 6. Optimise the number of tissue clusters using `K-means`.**
...
> **Step 7. Visualise the optimal tissue clusters.**
...
> **Step 9. Draw volcano plot showing differential interactions.**
...
> **Step 10. Construct and visualise a network of abundant interactions in cluster 5.**
...
inner_join(vertices, by="name") %>% as.data.frame() -> nodes_ab
...
ig_ab %>% xGGnetwork(node.label="label", node.label.size=1.5, node.xcoord="xcoord", node.ycoord="ycoord", node.size="degree", node.size.range=c(0.5,4), edge.color="lightblue", edge.arrow.gap=0) + guides(size="none") -> gg_up
gg_up
```

> **Step 11. Perform pathway enrichment analysis for genes involved in abundant interactions in cluster 5.**
...
> **Step 12. Conduct evolutionary analysis for genes involved in abundant interactions in cluster 5.**
